...
describe("Simple RT Timeline", () => {
  ...
  describe("timeline execution", () => {
    it("should display introduction instructions", async () => {
      ...
      expect(displayElement.innerHTML).toContain("Simple Reaction Time");
    });
  });
  ...
  describe("utils.scoring", () => {
    it("should calculate scores from empty data", () => {
      ...
      expect(scores.validTrials).toBe(0);
    ...
    it("should have correct scoring structure", () => {
      ...
      expect(scores).toHaveProperty("validTrials");
  ...
  describe("utils.getSummary", () => {
    it("should include task name and version", () => {
      ...
      expect(summary.taskName).toBe("simple-rt");
  ...
  describe("utils.constants", () => {
    it("should export task constants", () => {
      expect(utils.constants.TASK_NAME).toBe("simple-rt");
    ...
    it("should have valid default options", () => {
      ...
      expect(defaults.stimulusColor).toBe("#4A90D9");
    });
  });
  ...
  describe("configuration options", () => {
    it("should accept custom number of trials", () => {
      const jsPsych = initJsPsych();
      const timeline = createTimeline(jsPsych, { numTrials: 10 });
      expect(timeline.timeline).toBeDefined();
    });
    ...
    it("should accept custom response timeout", () => {
      const jsPsych = initJsPsych();
      const timeline = createTimeline(jsPsych, { responseTimeout: 2000 });
      expect(timeline.timeline).toBeDefined();
    });

    it("should accept custom stimulus color", () => {
      const jsPsych = initJsPsych();
      const timeline = createTimeline(jsPsych, { stimulusColor: "#00FF00" });
      expect(timeline.timeline).toBeDefined();
    });

    it("should accept custom stimulus size", () => {
      const jsPsych = initJsPsych();
      const timeline = createTimeline(jsPsych, { stimulusSize: 80 });
      expect(timeline.timeline).toBeDefined();
    });

    it("should accept custom min valid RT", () => {
      const jsPsych = initJsPsych();
      const timeline = createTimeline(jsPsych, { minValidRT: 150 });
      expect(timeline.timeline).toBeDefined();
    });

    it("should accept custom ITI", () => {
      const jsPsych = initJsPsych();
      const timeline = createTimeline(jsPsych, { iti: 750 });
      expect(timeline.timeline).toBeDefined();
    });
  });
