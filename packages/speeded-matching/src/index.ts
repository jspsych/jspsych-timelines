...
function createInstructions(
  instruction_pages: string[],
  next_button: string = "",
  back_button: string = ""
) {
  return {
    type: jsPsychInstructions,
    pages: instruction_pages.map(
      (page) => `<div class="jspsych-speeded-matching-instructions-container"><p>${page}</p></div>`
    ),
...
/**
 * Creates practice rounds with instructions and visual demonstrations
 * This helps participants understand the task before the actual trials
 */
function createPracticeRound(
  items: string[],
  num_choices: number = 4,
  practice_rounds: number = 1,
  practice_target_duration: number = 3000,
  text: typeof defaultText = defaultText
) {
  ...
  for (let round = 0; round < practice_rounds; round++) {
    ...
    // Practice trial with instruction practice_look_instruction
    practice_timeline.push({
      type: jsPsychHtmlButtonResponse,
      stimulus: `
    ...
    practice_timeline.push({
      ...
      // Same balanced grid layout as the main trials (see grid_columns note there).
      grid_columns: Math.ceil(Math.sqrt(practice_set.choices.length)),
      grid_rows: null,
      ...
      on_start: function () {},
      on_load: function () {
        // Set the choice count for CSS-driven dynamic sizing of each button.
        ...
        setTimeout(() => {
          const buttons = document.querySelectorAll(
            ".jspsych-speeded-matching-choice-option:not(.jspsych-speeded-matching-disabled-choice)"
          );
        ...
        setTimeout(() => {
          const incorrectButtons = document.querySelectorAll(
            ".jspsych-speeded-matching-choice-option.jspsych-speeded-matching-disabled-choice"
          );
...
function createFixationTrial(
  text: typeof defaultText = defaultText,
  interTrialInterval: number = 1000
) {
  return {
    type: jsPsychHtmlButtonResponse,
...
export function createTimeline(jsPsych: JsPsych, config: SpeedMatchingConfig = {}) {
  ...
  if (show_practice) {
    const practice_round = createPracticeRound(
      items,
      num_choices,
      practice_rounds,
      practice_target_duration,
      text
    );
  ...
  trials.forEach((trial, index) => {
    ...
    const mainTrial: any = {
      ...
      grid_columns: Math.ceil(Math.sqrt(trial.choices.length)),
      grid_rows: null,
      button_html: function (choice: string, choice_index: number) {
...
function calculatePerformance(data: any[]) {
  ...
  const mean_rt =
    valid_rts.length > 0 ? valid_rts.reduce((a, b) => a + b, 0) / valid_rts.length : null;
