        ...
        timeline: [
          {
            ...
            stimulus: () => {
              ...
              const digitIndex = jsPsych.data
                .get()
                .filter({ task: TASK_NAME, trial_type: "digit_presentation", parent_trial: trialNumber })
                .count();
...
function calculateModeScores(data: DataCollection, mode: "forward" | "backward"): ScoringResult {
  const testTrials = data
    .filter({ task: TASK_NAME, phase: "test", mode: mode, trial_type: "response" })
    .values() as TrialData[];
