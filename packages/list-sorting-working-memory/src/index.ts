...
// utils
function preload(stimulusSet: Array<Array<listSortingWorkingMemoryTestStimulusInfo>>) {
  return {
    type: jsPsychPreload,
    images: stimulusSet.flatMap((stimulus) =>
      stimulus.map((stimulusInfo) => stimulusInfo.stimulus_image)
    ),
    audio: stimulusSet.flatMap((stimulus) =>
      stimulus.map((stimulusInfo) => stimulusInfo.stimulus_audio)
    ),
  };
}
...
function lswmTrial(
  ...
  return {
    ...
    stimulus: options.stimulus_audio
      ? options.stimulus_audio
      : jsPsych.timelineVariable("stimulus_audio"),
    prompt: `<img src="${
      options.stimulus_image ? options.stimulus_image : jsPsych.timelineVariable("stimulus_image")
    }" />`,
    choices: "NO_KEYS",
    trial_duration: 2000,
  };
...
function lswmTrialSequence(
  ...
  return {
    ...
    timeline_variables: jsPsych.randomization.sampleWithoutReplacement(
      options.stimulus_set ? options.stimulus_set : jsPsych.timelineVariable("stimulus_set"),
...
function lswmSection(
  ...
  // Flatten stimulus_set
  const stimulusSetFlat = options.stimulus_set.flatMap((stimulus) =>
...
export function createTimeline(
  ...
  let timeline = [];
  timeline.push(preload(options.stimulus_set));
