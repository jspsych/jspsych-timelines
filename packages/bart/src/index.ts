...
function createInstructions(instruction_pages_data = instruction_pages) {
  return {
    ...
    pages: instruction_pages_data.map(
...
function showStartInstructions(
  ...
  currency_unit_per_pump: number = 1,
) {
...
function showEndResults(jsPsych: JsPsych, currencyFormatter?: CurrencyFormatter) {
  ...
  const stimulus = `<div class="instructions-container">
...
<div class="trial-container">
        ...
        const total_points =
          ...
          jsPsych.data
            .get()
            .filter({ task: "bart", exploded: false, cashed_out: true })
            .select("pump_count")
            .sum();
...
export function createTimeline(
  ...
  const trial = createTrialTimeline(
    jsPsych,
    max_pumps,
    min_pumps,
    currency_unit_per_pump,
    trial_timeout,
    enable_timeout,
    currencyFormatter,
  );
