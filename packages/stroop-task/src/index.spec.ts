...
describe("stroop-task utility functions", () => {
  ...
  describe("generateStimuli", () => {
    ...
    test("distributes 18 congruent trials across 4 colors (5, 5, 4, 4)", () => {
      ...
      const counts = colors.map((color) => congruent.filter((s) => s.word === color).length);
...
describe("stroop-task timeline units", () => {
  ...
  describe("createPracticeDebrief", () => {
    test("creates debrief with correct properties", () => {
      const debrief = timelineUnits.createPracticeDebrief("Practice complete!", "Start");
    ...
    test("has correct choices", () => {
      const debrief = timelineUnits.createPracticeDebrief("Practice complete!", "Start Experiment");
    ...
    test("uses provided stimulus text", () => {
      ...
      const debrief = timelineUnits.createPracticeDebrief(customText, "Continue");
...
describe("stroop-task full timeline", () => {
  ...
  test("includes instructions when enabled", () => {
    ...
    const hasInstructions = timelineWithInstructions.timeline.some(
      (trial: any) => trial.pages && Array.isArray(trial.pages)
    );
  ...
  test("excludes instructions when disabled", () => {
    ...
    const hasInstructions = timelineWithoutInstructions.timeline.some(
      (trial: any) => trial.pages && Array.isArray(trial.pages)
    );
  ...
  test("includes results when enabled", () => {
    ...
    const hasResults = timelineWithResults.timeline.some(
      (trial: any) => trial.choices && trial.choices.includes("Finish")
    );
  ...
  test("excludes results when disabled", () => {
    ...
    const hasResults = timelineWithoutResults.timeline.some(
      (trial: any) => trial.choices && trial.choices.includes("Finish")
    );
  ...
  test("creates practice trials when specified", () => {
    ...
    const practiceTrials = timeline.timeline.filter(
      (trial: any) => trial.data && trial.data.phase === "practice"
    );
